      ...
      <div className="mt-[30px] flex flex-col items-center gap-5 lg:grid-cols-2 lg:grid">
        {product.general_params.length > 0 &&
          product.general_params.map((el) => {
            if (!el.icon || !el.title) return;
            return (
              <div key={el.id} className="flex items-center w-full gap-2 bg-white h-[60px] px-2.5 rounded-xl lg:h-20">
                <Image
                  src={strapiUrl + el.icon.url}
                  alt="icon"
                  width={80}
                  height={80}
                  className="h-[90%] w-auto aspect-square object-contain"
                />
      ...
      {product.variation && (
        <div className="mt-5 lg:mt-10">
          <div className="flex flex-col justify-between gap-5 xs:gap-3 xs:items-center xs:flex-row">
            {product.variation.length > 0 && <h2 className="font-semibold text-[18px] lg:text-[22px]">Все модели</h2>}
            <a
              href="#productParams"
              className="font-semibold text-[14px] lg:text-[16px] rounded-full underline underline-offset-4 hover:no-underline"
            >
              {product.variation.length > 0 ? "Посмотреть характеристики модели" : "Посмотреть характеристики"}
            </a>
          </div>
